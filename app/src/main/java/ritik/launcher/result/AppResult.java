...
import ritik.launcher.utils.SpaceTokenizer;
...
//        }
    ...
    protected PopupMenu buildPopupMenu(Context context, final RecordAdapter parent, View parentView) {
        ...
        if (Application.getRootHandler(context).isRootActivated() && Application.getRootHandler(context).isRootAvailable()) {
    ...
    private void excludeFromAppList(Context context, AppPojo appPojo) {
        Application.getDataHandler(context).addToExcluded(appPojo.packageName, appPojo.userHandle);
        //remove app pojo from appProvider results - no need to reset handler
        Application.getDataHandler(context).getAppProvider().removeApp(appPojo);
        Application.getDataHandler(context).removeFromFavorites((MainActivity) context, appPojo.id);
    ...
    private void launchEditTagsDialog(final Context context, final AppPojo app) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(context,
                android.R.layout.simple_dropdown_item_1line, Application.getDataHandler(context).getTagsHandler().getAllTagsAsArray());
        ...
        builder.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                Application.getDataHandler(context).getTagsHandler().setTags(app.id, tagInput.getText().toString());
    ...
    private void hibernate(Context context, AppPojo app) {
        ...
        if (!Application.getRootHandler(context).hibernateApp(appPojo.packageName)) {
    ...
    public Drawable getDrawable(Context context) {
        ...
        if (icon == null) {
             icon = Application.getIconsHandler(context).getDrawableIconForPackage(className, this.appPojo.userHandle);
        }
