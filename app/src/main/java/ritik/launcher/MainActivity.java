...
import ritik.launcher.adapter.ViewPageradapter;
import ritik.launcher.broadcast.IncomingCallHandler;
import ritik.launcher.broadcast.IncomingSmsHandler;
import ritik.launcher.dataprovider.AppProvider;
import ritik.launcher.pojo.Pojo;
import ritik.launcher.result.Result;
import ritik.launcher.searcher.ApplicationsSearcher;
import ritik.launcher.searcher.HistorySearcher;
import ritik.launcher.searcher.NullSearcher;
import ritik.launcher.searcher.QueryInterface;
import ritik.launcher.searcher.QuerySearcher;
import ritik.launcher.searcher.Searcher;
import ritik.launcher.ui.BlockableListView;
import ritik.launcher.ui.BottomPullEffectView;
import ritik.launcher.ui.KeyboardScrollHider;
import ritik.launcher.utils.PackageManagerUtils;

public class MainActivity extends Activity implements QueryInterface, KeyboardScrollHider.KeyboardHandler {

    public static final String START_LOAD = "ritik.summon.START_LOAD";
    public static final String LOAD_OVER = "ritik.summon.LOAD_OVER";
    public static final String FULL_LOAD_OVER = "ritik.summon.FULL_LOAD_OVER";
    /**
     ...
     */
    public RecordAdapter adapter,adapter2;
    /**
    ...
    private View Bar;
    /**
     * Favorites bar, in the bar (not the quick favorites bar from minimal UI)
     */
   // private View favoritesBar;

    /**
     * Task launched on text change
     */
    private Searcher searcher;
    private ApplicationsSearcher applicationsSearcher;
    /**
     * Search edit layout
     */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (intent.getAction().equalsIgnoreCase(LOAD_OVER)) {
                    ...
                    System.gc();
                    Log.d(TAG, "onReceive: FULL_LOAD_OVER");
                   // displayQuickFavoritesBar(true, false);
                    ...
                    displayLoader(true);
                    Log.d(TAG, "onReceive: START_LOAD_OVER");
                }
            }
        };
        ...
        Application.initDataHandler(this);
        ...
        this.list = (GridView) this.findViewById(android.R.id.list);
        this.list2=(GridView)this.findViewById(R.id.list2);
        list.setNumColumns(Integer.parseInt(prefs.getString("col-values","4")));
        list2.setNumColumns(Integer.parseInt(prefs.getString("col-values","4")));
...
viewPager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        Log.d(TAG, "onPageScrolled: pixels "+positionOffsetPixels+" offset "+positionOffset+" position "+position);
if(position==0 && positionOffset>0.5){
    listEmpty.setVisibility(View.GONE);
        ...
        if(position!=1){
            ...
            Bar.setAlpha(positionOffset);

            ViewGroup.LayoutParams layoutParams= Bar.getLayoutParams();
      layoutParams.width=positionOffsetPixels;
    ...
    public void onPageSelected(int position) {
        Log.d(TAG, "onPageSelected: favourites are "+tryToRetrieve+"       "+favsIds.length);
        if(position==0){
         ...
         menuButton.setVisibility(View.VISIBLE);
           ...
           //TODO try not to get the list affected at the first time
          //notify when editext changes por it has content when page scrolled
...
searchEditText.setVisibility(View.GONE);
    ...
    public void onPageScrollStateChanged(int state) {
        ...
        this.adapter = new RecordAdapter(this, this, R.layout.item_app, new ArrayList<Result>(),true);
        this.list.setAdapter(this.adapter);
        this.adapter2 = new RecordAdapter(this, this, R.layout.item_app, new ArrayList<Result>(),false);
        ...
        Bar = findViewById(R.id.main_bar);
       // favoritesBar = findViewById(R.id.favoritesBar);
        ...
        this.list.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
        ...
        this.list2.setLongClickable(true);
        this.list2.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            @Override
            public boolean onItemLongClick(AdapterView<?> parent, View v, int pos, long id) {
    ...
    private void registerLongClickOnFavorites() {
        View.OnLongClickListener listener = new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View view) {
                ...
                ArrayList<Pojo> favorites = Application.getDataHandler(MainActivity.this).getFavorites(tryToRetrieve);
...
//                favoritesBar.setVisibility(View.INVISIBLE);
//                displayFavorites();
//            }
//        } else {
...
//    }
    ...
    protected void onResume() {
        ...
        Bar.getLayoutParams().height = searchHeight;
        ...
        AppProvider appProvider = Application.getDataHandler(this).getAppProvider();
        if (appProvider != null && appProvider.isLoaded())
            ...
            // (not their intent) after moving to another activity and switching back
    ...
    protected void onDestroy() {
        ...
        Application.getCameraHandler().releaseCamera();
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        // onResume() will be called right after, hiding the bar if any.
    ...
    public void onBackPressed() {
        ...
        if (Bar.getVisibility() == View.VISIBLE) {
            ...
            // If no menu, empty the search bar
            searchEditText.setText("");
        }
    ...
    public void onMenuButtonClicked(View menuButton) {
        // When the  bar is displayed, the button can still be clicked in a few areas (due to favorite margin)
        // To fix this, we discard any click event occurring when the bar is displayed
       // if (Bar.getVisibility() != View.VISIBLE)
            menuButton.showContextMenu();
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if ((event.getAction() == MotionEvent.ACTION_CANCEL) || (event.getAction() == MotionEvent.ACTION_UP)) {
            ...
            if (prefs.getBoolean("history-hide", false) && prefs.getBoolean("history-onclick", false)) {
                ...
                if ((Bar.getVisibility() != View.VISIBLE) && (searchEditText.getText().toString().isEmpty())) {
                    //if list is empty
                    if ((this.list.getAdapter() == null) || (this.list.getAdapter().getCount() == 0)) {
                        searcher = new HistorySearcher(MainActivity.this);
                        Log.d("Ritik", "Searcher History Searcher");
                        searcher.execute();
                    }
                }
            }
    ...
    public void onSearchButtonClicked(View clearButton) {
        Uri uri = Uri.parse("http://www.google.com/search?q="+searchEditText.getText().toString().replace(' ','+'));
  ...
  startActivity(intent);
    ...
    public void onLauncherButtonClicked(View launcherButton) {
        // Display or hide the  bar, according to current view tag (showMenu / hideMenu).

        displayBar(launcherButton.getTag().equals("showMenu"));
    }
    ...
    private void displayBar(Boolean display) {
        Log.d(TAG, "displayBar: "+display);
...
//
       ...
       // int finalRadius = Math.max(Bar.getWidth(), Bar.getHeight());
...
//            }
        //    searcher = new ApplicationsSearcher(MainActivity.this);
          //  searcher.execute();
            ...
            if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
               ...
               Animator anim = ViewAnimationUtils.createCircularReveal(Bar, Bar.getWidth()/2, Bar.getHeight()/2, 0, Bar.getWidth()/2);
           ...
           //if (favoritesBar.getVisibility() == View.VISIBLE) {
           //if(false){
            ...
            if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                Animator anim = ViewAnimationUtils.createCircularReveal(Bar, Bar.getWidth()/2, Bar.getHeight()/2, Bar.getWidth()/2, 0);
        ...
        // Hide the favorite bar in the  bar if the quick bar is enabled
    ...
    //    int[] favoritesIds = favoritesBar.getVisibility() == View.VISIBLE ? favsIds : favBarIds;
        //todo look here
        int[] favoritesIds = favsIds;

        ArrayList<Pojo> favoritesPojo = Application.getDataHandler(MainActivity.this)
    ...
    public void LoadLauncher(){
        Log.d("Ritik", "Searcher Application custom in loadlauncher ");
        applicationsSearcher=new ApplicationsSearcher(MainActivity.this);
        applicationsSearcher.execute();
        appsLoaded=true;
    }
}
